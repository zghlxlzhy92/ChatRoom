...
import hz44_rx4.server.sModel.Team;
...
public class MiniModel implements IChatroom{
    ...
    private Team team;
    private Timer timer = new Timer(200, (e)->MiniModel.this.check());
   ...
   public void startGame(){
	    if(start) return;
	    	...
	    	start = true;
		    UUID gameID = this.getId();
            team = new Team(this.getChatServers());
		    for(IChatServer player : this.getChatServers()){
			    try {
			    	player.receive(new StartGame(new GameFactory(hostServer, player, gameID), hostServer).getDataPacket());
   ...
   public void check(){
 		  if(team.result == 600 && team.replier.size() != team.player.size()){
					...
					for(IChatServer client : team.player){
						  try {
							client.receive(new ShowRank(score, hostServer).getDataPacket());
						} catch (Exception e) {
